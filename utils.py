...
def gen_daily_sharpe(pf_returns, r_f=1.0):
    'Returns daily sharpe ratio from pandas series of asset returns'
...
def momentum_strat(data, lookback_window=14, holding_period=14, param_dict=None):
    ...
    # Make momentum factor
    technicals_df = data['EndofDayPrice'].groupby(level=1).pct_change(lookback_window)
    technicals_df = technicals_df[lookback_window+1:]  # Slice off 'warm-up' period
    technicals_df.dropna(inplace=True)  # Remove new players without sufficient price lookback 
...
def mean_reversion(data, lookback_window=14, holding_period=14, param_dict=None):
    ...
    technicals_df = data['EndofDayPrice'].groupby(level=1).pct_change(lookback_window)  # Make momentum factor
    technicals_df.dropna(inplace=True)  # Remove new players without sufficient price lookback 
...
def post_div_drift(data, holding_period=14, param_dict=None):
    'Buy players who have just had performance dividends issued'
    # If dictionary passed, use those parameters instead
    if param_dict:
        holding_period = param_dict['holding_period']
    
    technicals_df = data['MatchdayDividends'].copy()
